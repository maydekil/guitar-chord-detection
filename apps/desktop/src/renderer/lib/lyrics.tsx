...
import { normalizeLyricDisplayText } from "@gcd/shared/lyrics";
...
export function parseLyrics(lyrics: string): LyricLine[] {
    return lyrics
        ...
        .map((line) => {
            ...
            if (!match) {
                return { time: null, text: normalizeLyricDisplayText(line) };
            }
            const minutes = Number.parseInt(match[1], 10);
            const seconds = Number.parseInt(match[2], 10);
            const fraction = match[3] ? Number.parseFloat(`0.${match[3].padEnd(3, "0")}`) : 0;
            const time = minutes * 60 + seconds + fraction;
            return { time: Number.isFinite(time) ? time : null, text: normalizeLyricDisplayText(match[4].trim()) };
        });
}
